...
const Dashboard: React.FC = () => {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      socket.on("connect", () => {
        console.log(
          `WebSocket connected! Socket ID: ${socket.id}, Job ID: ${jobId}`
        );
      });

      socket.on("progress-update", (data) => {
        console.log(
          `Progress update: ${data.progress || 0}% - Status: ${data.status}`
        );
      ...
      console.log(
        "FileConvert API request sent. Progress will come via WebSocket."
      );
    } catch (error) {
      console.error("Conversion error:", error);
      setErrorMessage(
            ...
            {selectedFile && fileStats && (
              <div className="bg-gray-50">
                ...
                <button
                  ...
                  className={`w-full py-3 sm:py-4 text-white rounded-lg transition font-semibold text-base shadow-l ${
                    fileStats.totalFiles > 20
                      ? "cursor-not-allowed bg-[#E46356]/50"
                      : "cursor-pointer bg-[#E46356]"
                  }`}
                >
                  Start Conversion
                </button>
                {fileStats.totalFiles > 20 && (
                  <p className="p-2 text-[#E46356] text-center">
                    Too many SQL/PLSQL files — please split and reupload the
                    ZIP.
                  </p>
                )}
              </div>
              ...
              <div className="flex flex-col text-center items-center gap-3">
                ...
                <h3 className=" manrope-medium text-md text-gray-900">
                  {progress < 30
                    ? "Analyzing dependencies......"
                    : progress < 80
                    ? "Converting SQL/PLSQL files to Snowflake JavaScript files"
                    : "Finalizing project..."}
                </h3>
              </div>
            </div>
              ...
              <div className="flex flex-col sm:flex-row items-center max-w-4xl bg-white justify-center mb-6 gap-4 p-10 shadow-lg rounded-xl">
                <div className="flex flex-col justify-center gap-3">
                  ...
                  <h1 className="text-center text-md">
                    Your Oracle SQL/PLSQL project has been successfully
                    converted to Snowflake JavaScript stored procedures.
                  </h1>
                  <p className="text-gray-600 text-center text-sm">
                    Successfully converted{" "}
                    {convertedFile.conversion.convertedFiles.length} files
                  </p>
                </div>
              </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm px-4">
                  <div className="relative bg-white rounded-2xl shadow-2xl border border-gray-200 w-full max-w-6xl max-h-[90vh] overflow-y-auto p-6">
                    ...
                    <h1 className="w-full text-center text-sm mb-5 manrope-regular">
                      Review the original Oracle SQL/PLSQL code alongside the
                      converted Snowflake JavaScript output
                    </h1>
                      ...
                      <div className="bg-neutral-50 rounded-2xl max-h-[66vh]">
                        <div className="flex items-center gap-2 mb-3 p-3 bg-neutral-200 rounded-t-2xl">
                          ...
                          <h4 className="font-regular text-gray-900">
                            Original Oracle SQL/PLSQL Code
                          </h4>
                        </div>
                      ...
                      <div className="bg-neutral-50 rounded-2xl max-h-[66vh]">
                        <div className="flex items-center gap-2 mb-3 p-3 bg-black rounded-t-2xl">
                          ...
                          <h4 className="font-regular text-white">
                            Converted Snowflake JavaScript Code
                          </h4>
                        </div>
                        <div className="space-y-3 max-h-[55vh] overflow-y-auto pr-2 p-5">
                          {convertedFile.conversion.convertedFiles.map(
